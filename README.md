...
다음 작업을 시작할 때 알아서 올린다. 사람이 따로 받을 것도 없다 — 다른 PC가 고친 파일은 OneDrive 가 맞춰 두고,
클라우드 세션(claude.ai/code)에서 고쳐 main 에 머지한 것은 그 PC의 AI 가 작업을 시작할 때 받아 온다(`공통/scripts/receive.py`). 절차는 `공통/운영.md` '시스템 파일 올리기' 에 있다. PC마다 처음 한 번 하는 일(`g.bat` 실행 허락)은 `시작하기.md` 에 있다.
