...
# 해라체: 하십시오체는 모두 '니다.'로 끝나므로 그 밖의 '다.'를 잡는다. 개요 기호는 뺀다.
PLAIN = re.compile(r'(?<!니)다\.(?:\s|$)')
OUTLINE = re.compile(r'^[가-하]\.\s')
# 증거 인용으로 끝나는 사실 문단 뒤에는 지시 접속어로 시작하는 평가 문단이 온다.
CITED = re.compile(r'(갑|을|을가|을나|병|정|노|사)\s*제\s*\d+호증[^)]*\)\s*\.?\s*$')
...
def split_sentences(text):
    ...
    return [p for p in parts if SENTENCE.search(p)]
...
def check(path: Path):
    ...
    for line, text, body in blocks:
        if not body:                      # 보완 메모는 제출용 본문이 아니다
            previous_cited = False
            continue
        for hit in set(BANNED.findall(text)):
            add(violations, '금지 낱말', line, text, hit)
        if CONJUNCTION.match(text):
        ...
        plain = PLAIN.search(text)
        if plain and not OUTLINE.match(text):
        ...
        if NESTED.search(text):
            add(notes, '괄호 안 내부 소괄호', line, text,
                NESTED.search(text)[0][:40])
        previous_cited = bool(CITED.search(text))
    ...
    body_text = [t for _, t, body in blocks if body]
