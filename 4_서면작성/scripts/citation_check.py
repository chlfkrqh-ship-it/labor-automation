...
RANGE = re.compile(r'(%s)\s*[~∼-]\s*(%s)' % (_system.EVIDENCE_NUMBER, _system.EVIDENCE_NUMBER))
PARTS = re.compile(r'^(갑|을가|을나|을|병|정|노|사)제(\d+)호증(?:의(\d+))?$')
# 목록 표 첫 칸의 약식 표기: '을나1-1', '갑14', '사 제3호증' 을 모두 받는다.
SHORT = re.compile(r'^(갑|을가|을나|을|병|정|노|사)\s*(?:제\s*)?(\d+)(?:\s*[-의]\s*\d+)?\s*(?:호증)?$')
...
def parent(label):
    """'갑제57호증의1' 은 '갑제57호증' 으로 묶어 본다."""
    m = PARTS.match(normalize(label))
    return m[1] + '제' + m[2] + '호증' if m else normalize(label)
...
def catalogues(case_dir: Path):
    ...
    노동위원회 단계는 사·근(또는 노·사) 체계, 행정소송 단계는 갑·을 체계로 호증을
    다시 매기므로 목록이 여러 벌 있다. 한 벌만 보면 다른 체계의 인용이 전부
    '목록에 없음'으로 잡힌다.
    """
...
def listed(case_dir: Path):
    ...
    for p in catalogues(case_dir):
        ...
        for row in p.read_text(encoding='utf-8-sig').split('\n'):
            ...
            for start, end in RANGE.findall(row):
                a, b = PARTS.match(normalize(start)), PARTS.match(normalize(end))
                if a and b and a[1] == b[1] and int(a[2]) <= int(b[2]):
                    for n in range(int(a[2]), int(b[2]) + 1):
                        result.setdefault('%s제%d호증' % (a[1], n), name)
            for hit in EVIDENCE.findall(row):
                result.setdefault(parent(hit), name)
    return result
...
def recorded_cases():
    ...
    if folder.is_dir():
        for p in folder.glob('*.md'):
            found |= set(CASE.findall(p.stem + ' ' + p.read_text(encoding='utf-8-sig')))
    return found
...
def check(draft: Path, case_dir: Path):
    ...
    cited = {}
    for hit in EVIDENCE.findall(text):
        cited.setdefault(parent(hit), set()).add(normalize(hit))
